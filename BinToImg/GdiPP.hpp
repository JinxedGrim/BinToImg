...
#define PixelRoundCeil(Val) (int)std::ceilf(Val)
...
#pragma omp parallel for
    ...
    void __inline __fastcall QuickSetPixel(const int& StartIdx, COLORREF Clr)
    {
        PBYTE pixelPtr = &PixelBuffer[StartIdx];
        *pixelPtr++ = GetBValue(Clr);
        *pixelPtr++ = GetGValue(Clr);
        *pixelPtr = GetRValue(Clr);
        this->NeedsPixelsDrawn = true;
    ...
    */
    ...
    const bool __inline __fastcall DrawDoubleBuffer()
    {
